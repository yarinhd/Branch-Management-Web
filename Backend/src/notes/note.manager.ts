...
export default class NoteManager {
     ...
     */
    static async getAllUserNotes(userId: string): Promise<INote[]> {
        const foundNotes: INote[] | null = await NoteRepository.getAllUserNotes(userId);
        if (!foundNotes) {
     ...
     */
    static async createNote(newNote: Partial<INote>) {
