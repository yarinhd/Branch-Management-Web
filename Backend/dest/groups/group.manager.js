...
class GroupManager {
     ...
     */
    static async createGroupFromPeopleApi(userId, name, parentName) {
        if (!userId) {
        ...
        const addedGroup = await group_repository_1.default.createGroup(group);
        if (!addedGroup) {
    ...
    static async deletedGroupByName(GroupName) {
        ...
        const deletedGroup = await (0, utils_1.recursiveGroupDelete)(populatedGroup);
        if (!deletedGroup) {
