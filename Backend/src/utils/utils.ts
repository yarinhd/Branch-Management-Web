...
import {
    BranchManagerNotFoundError,
    LackOfUserInfoError,
    TeamUsersNotFoundError,
    UserNotFoundError,
...
export async function createBranch(branch: string) {
    ...
    const createdBranchGroups: string[] | null = await GroupManager.getAllBranchHierarchyNames(branch);
    if (createdBranchGroups) {
        await GroupManager.deletedManyGroupByName(createdBranchGroups);
    }
    let branchUsers: IPeopleUser[] = await users.getUsers(branch, config.peopleApi.UserAndGroupfields);
    ...
    await GroupManager.createGroupFromPeopleApi(branchManagerUser._id as string, branchManager.branch, null);
    ...
    await Promise.all(
        madorManagers.map((madorManagerUser) => {
            ...
            const createdGroup: Promise<IGroup> = GroupManager.createGroupFromPeopleApi(
    ...
    await Promise.all(
        teamManager.map(async (teamManagerUser) => {
            ...
            const createdGroup = GroupManager.createGroupFromPeopleApi(
                managerUser._id as string,
                teamManagerUser.team,
    ...
    const createdTeamUsers: IUser[] | null = await Promise.all(
        teamsUsers.map((teamUsersGroup) =>
            UserManager.upsertUserPersonalAndGroupInfo(teamUsersGroup, teamUsersGroup.team)
        )
    );
    if (!createdTeamUsers) {
        throw new Error();
...
export async function updateBranch(BranchName: string) {
    ...
    const originalBranchUsers: IUser[] = await UserManager.getAllBranchUsers(BranchName);
...
export async function getAllBranchUsers(branchName: string) {
